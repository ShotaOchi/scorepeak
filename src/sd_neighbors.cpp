...
Rcpp::NumericVector sd_neighbors_left_reflecting_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  double sum_squared_window = 0.0;
  
  //' create reflecting left window
  for (int i = windowsize_half; i > 0; --i)
  {
    sum_window += data[i];
    sum_squared_window += data[i] * data[i];
  }
  
  //' store sd of temporal left neighbors
  res[0] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize_half);
  ...
  for (int i = 1; i < n; ++i)
  {
    ...
    sum_squared_window += data[max_range] * data[max_range];
    sum_squared_window -= data[out_range] * data[out_range];
    res[i] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize_half);
  }
  return res;
}
...
Rcpp::NumericVector sd_neighbors_right_reflecting_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  //' store sd of temporal right neighbors
  res[0] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize_half);
...
Rcpp::NumericVector sd_neighbors_both_reflecting_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  //' store sd of temporal neighbots
  res[0] = sd_temporal_neighbors(sum_window, sum_squared_window, double_windowsize_half);
  
  // store sd of temporal neighbors
  for (int i = 1; i < n; ++i)
  {
    //' left window
    ...
    sum_squared_window += data[max_range_left] * data[max_range_left];
    sum_squared_window -= data[out_range_left] * data[out_range_left];
    //' right window
    ...
    sum_squared_window += data[max_range_right] * data[max_range_right];
    sum_squared_window -= data[out_range_right] * data[out_range_right];
    res[i] = sd_temporal_neighbors(sum_window, sum_squared_window, double_windowsize_half);
  }
  return res;
}
...
Rcpp::NumericVector sd_neighbors_all_reflecting_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  //' store sd
  res[0] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize);
  ...
  for (int i = 1; i < n; ++i)
  {
    ...
    sum_squared_window += data[max_range] * data[max_range];
    sum_squared_window -= data[out_range] * data[out_range];
    res[i] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize);
  }
  return res;
}
...
Rcpp::NumericVector sd_neighbors_left_discard_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  if (n >= windowsize)
  {
    ...
    //' store sd of temporal left neighbors
    res[windowsize_half] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize_half);
...
Rcpp::NumericVector sd_neighbors_both_discard_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  if (n >= windowsize)
  {
    ...
    //' store sd of temporal neighbots
    res[windowsize_half] = sd_temporal_neighbors(sum_window, sum_squared_window, double_windowsize_half);
...
Rcpp::NumericVector sd_neighbors_all_discard_cpp(const Rcpp::NumericVector& data, const int& windowsize)
{
  ...
  if (n >= windowsize)
  {
    ...
    //' store sd
    res[windowsize_half] = sd_temporal_neighbors(sum_window, sum_squared_window, windowsize);
  
    // calculate sd of temporal neighbors
    for (int i = windowsize_half + 1; i < nminwshalf; ++i)
    {
      int max_range = i + windowsize_half;
      int out_range = i - windowsize_half - 1;
